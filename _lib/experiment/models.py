...
def dragonnet_loss_binarycross_2(concat_true, concat_pred):
    return regression_loss(concat_true, concat_pred) - binary_classification_loss(concat_true, concat_pred)
...
def make_multimession(input_dim, reg_l2):
    """
    Neural net predictive model. The dragon has three heads.
    :param input_dim:
    :param reg:
...
def make_multidragonnet(input_dim, reg_l2):
    """
    Neural net predictive model. The dragon has three heads.
    :param input_dim:
    :param reg:
    ...
    y0_hidden = Dense(units=100, activation='elu', kernel_regularizer=regularizers.l2(reg_l2))(y0_hidden)
    y1_hidden = Dense(units=100, activation='elu', kernel_regularizer=regularizers.l2(reg_l2))(y1_hidden)
    y2_hidden = Dense(units=100, activation='elu', kernel_regularizer=regularizers.l2(reg_l2))(y1_hidden)
    ...
    y1_predictions = Dense(units=1, activation=None, kernel_regularizer=regularizers.l2(reg_l2), name='y1_predictions')(
        y1_hidden)
    y2_predictions = Dense(units=1, activation=None, kernel_regularizer=regularizers.l2(reg_l2), name='y2_predictions')(
        y2_hidden)
...
def make_combine_network(input_dim, reg_l2):
    """
    Neural net predictive model. The dragon has three heads.
    :param input_dim:
    :param reg:
    ...
    # representation
    tx = Dense(units=200, activation='elu', kernel_initializer='RandomNormal', name='ned_hidden1')(inputs)
    tx = Dense(units=200, activation='elu', kernel_initializer='RandomNormal', name='ned_hidden2')(tx)
    tx = Dense(units=200, activation='elu', kernel_initializer='RandomNormal', name='ned_hidden3')(tx)


    y_predictions = Dense(units=1, activation=None, name='ned_y_prediction')(tx)


    model = Model(inputs=inputs, outputs=y_predictions)
    return model
