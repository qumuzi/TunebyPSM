...
def train_and_predict_multidragons(t_train, y_unscaled, x_train, x_test,t_flags,
                                   knob_loss=dragonnet_loss_binarycross, val_split=0.2, batch_size=64):
    verbose = 0
    y_scaler = StandardScaler().fit(y_unscaled)
...
def turn_knob(data_base_dir, Workload, n_iter_sel, t_flags:list = None,
    ...
    for tar_flag in t_flags:
        Y[tar_flag] = pd.read_csv(
            BaseAddr+'OriginData/perf/{}/{}.csv'.format(ac_dict[tar_flag], Workload)
            ,index_col=0)[Outcome].values
        AverPerf[tar_flag] = np.mean(Y[tar_flag])

    def _cal_ra(input_dict):
        res = {}
        for i in list(AverPerf.keys()):
            res[i] = abs(input_dict[i]-AverPerf[i])/AverPerf[i]
        return res
    ...
    for idx in np.arange(TuningTime):
        t, y, x_train, ids = _read_data(
            data_base_dir, idx, t_flags, list(np.arange(len(t_flags))), [n_iter_sel]*len(t_flags))
        aver_perf = {}
        for i in np.arange(len(t_flags)):
            aver_perf[t_flags[i]] = np.mean(y[i*n_iter_sel:(i+1)*n_iter_sel])
        cal_tmp["Aver"].append(_cal_ra(aver_perf))

        cal_tmp["MulDra"].append(_cal_ra(train_and_predict_multidragons(
            t, y, x_train, x_test,t_flags, knob_loss=knob_loss,val_split=0.2, batch_size=64)))

        cal_tmp["Linear"].append(_cal_ra(train_and_predict_linear(t, y, x_train, x_test,t_flags)))

        cal_tmp["Cart"].append(_cal_ra(train_and_predict_rf(t, y, x_train, x_test,t_flags)))
    ...
    for key in list(cal_tmp.keys()):
        ...
        cal_list = []
        for env in t_flags:
            for item in cal_tmp[key]:
                cal_list.append(item[env])
        Res[key] = np.mean(cal_list)
...
def main():
    ...
    for workload in Workloads:
        ...
        for sampled_num in np.arange(3,31,3):
            ...
            Res[workload+"+"+str(sampled_num)] = turn_knob(
                BaseAddr+'SingleEnvTrack/generate/'+workload+'/rlr/',
                workload, sampled_num, t_flags=list(ac_dict.keys()))

    print(Res)
    with open('MultipleEnvTrack/RQ3/RA.txt', 'w') as f:
        f.write(str(Res))
